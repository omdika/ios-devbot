            ...
            if file.endswith(allowed_exts) or file == "Dockerfile":
                ...
                try:
                    with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                        content = f.read()
                        if len(content) < 50000: # Lewati file autogenerated yang terlalu besar
...
def apply_ai_changes_multimodal(user_id: int, prompt: str, images_bytes_list: list = None) -> str:
    ...
    system_instruction = (
        ...
        "Jika ada gambar UI / screenshot yang dilampirkan bersama prompt, jadikan gambar tersebut sebagai acuan utama UI/UX dan alur transaksi.\n\n"
        "ATURAN FORMAT OUTPUT UNTUK FILE BARU / FILE EDIT (Hanya tulis file yang relevan):\n"
        "Untuk Backend:\n"
        "REPO: backend\n"
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ALLOWED_USER_ID:
        ...
        return
    await update.message.reply_text("👋 Halo! AI Agent siap dengan Single-Batch Processor.")
...
async def execute_task(chat_id: int, user_id: int, prompt: str, images: list, message_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        img_info = f" beserta {len(images)} screenshot" if images else ""
        status_msg = await context.bot.send_message(
            chat_id=chat_id,
            text=f"🔄 [1/4] Memproses instruksi{img_info}..."
        )
        ...
        try:
            ...
            await status_msg.edit_text("🧠 [2/4] Vertex AI sedang menganalisis gambar & menulis kode...")
            explanation = apply_ai_changes_multimodal(user_id, prompt, images)
            ...
            await status_msg.edit_text(
                f"✅ Tugas Selesai! (1 Commit Bersih)\n\n"
...
async def handle_incoming(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ambil bytes gambar jika ada
    img_bytes = None
    if message.photo:
        photo_file = await message.photo[-1].get_file()
        img_bytes = bytes(await photo_file.download_as_bytearray())

    # Inisialisasi antrean pesan untuk user
    ...
    # Batalkan timer sebelumnya jika ada pesan baru yang masuk dalam jeda singkat
    if queue["timer_task"] and not queue["timer_task"].done():
        queue["timer_task"].cancel()

    async def delayed_trigger():
        await asyncio.sleep(2.5) # Tunggu 2.5 detik untuk mengumpulkan semua teks & gambar
...
def main():
    ...
    print("🤖 AI Agent siap dengan Single-Batch Queue...")
    app.run_polling()
